...
from maskrcnn_benchmark.structures.ops import nms, soft_nms, softer_nms
from maskrcnn_benchmark.structures.ops import remove_small_boxes

from ..utils import cat
from .utils import permute_and_flatten

use_nms = ['nms', 'soft_nms', 'softer_nms'][0]
...
class RPNPostProcessor(torch.nn.Module):
    ...
    def forward(self, anchors, objectness, box_regression, targets=None):
        ...
        if num_levels > 1:
            boxlists = self.select_over_all_levels(boxlists, post_nms_top_n=post_nms_top_n)
    ...
    def select_over_all_levels(self, boxlists, post_nms_top_n=None):
        ...
        if self.training and self.fpn_post_nms_per_batch:
            ...
            post_nms_top_n0 = post_nms_top_n
            for i in range(num_images):
                objectness = boxlists[i].get_field("objectness")
                if post_nms_top_n0 is None:
                    post_nms_top_n = min(self.fpn_post_nms_top_n, len(objectness))
                else:
                    post_nms_top_n = post_nms_top_n0
