...
def draw_bboxes(img, bbox, identities=None, offset=(0,0)):
    ...
    for i,box in enumerate(bbox):
        ...
        cv2.rectangle(img_temp,(x1, y1),(x2,y2),color,-1)

    alpha = 0.5
    img = cv2.addWeighted(img, 1 - alpha, img_temp, alpha, 0)
    ...
    for i,box in enumerate(bbox):
        ...
        # box text and bar
        id = int(identities[i]) if identities is not None else -1
        label = '{}{}'.format("#", id)
        t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1 , 1)[0]
        cv2.putText(img,label,(int(x1+(x2-x1)/2-t_size[0]/2),y1+t_size[1]+10), cv2.FONT_HERSHEY_PLAIN, 1.5, [255,255,255], 2)

    return img


def draw_bboxes0(img, bbox, color=(144,238,144)):
    for i,box in enumerate(bbox):
        x1,y1,x2,y2 = [int(i) for i in box]
        cv2.rectangle(img,(x1, y1),(x2,y2),color,2)
    return img
